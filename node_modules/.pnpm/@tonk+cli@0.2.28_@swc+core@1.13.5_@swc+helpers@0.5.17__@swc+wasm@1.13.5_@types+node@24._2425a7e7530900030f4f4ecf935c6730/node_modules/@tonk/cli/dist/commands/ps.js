...
export const psCommand = new Command('ps')
    ...
    try {
        ...
        console.log(`${chalk.bold('ID'.padEnd(36))} | ${chalk.bold('Bundle'.padEnd(20))} | ${chalk.bold('Route/Port'.padEnd(15))} | ${chalk.bold('Status')}`);
        console.log('-'.repeat(85));
        servers.forEach((server) => {
            const routeOrPort = server.route || (server.port ? `:${server.port}` : 'N/A');
            console.log(`${server.id.padEnd(36)} | ${server.bundleName.padEnd(20)} | ${routeOrPort.padEnd(15)} | ${server.status}`);
        });
        const duration = Date.now() - startTime;
        trackCommandSuccess('ps', duration, {
            serverUrl,
        ...
        const duration = Date.now() - startTime;
        trackCommandError('ps', error, duration, {
            serverUrl,
        });
