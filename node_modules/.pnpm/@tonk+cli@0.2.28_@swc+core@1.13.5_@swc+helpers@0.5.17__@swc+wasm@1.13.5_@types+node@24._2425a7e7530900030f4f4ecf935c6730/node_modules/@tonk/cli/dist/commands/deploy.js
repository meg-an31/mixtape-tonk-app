...
function readPackageJson() {
    ...
    if (!fs.existsSync(packagePath)) {
        throw new Error('package.json not found. Are you in a Tonk project directory?');
    }
    try {
 ...
 */
function determineAppName(options, tonkConfig, packageJson) {
    if (options.name) {
        return options.name;
    }
    if (tonkConfig?.name) {
 ...
 */
async function buildApp(skipBuild, bundleName) {
    if (skipBuild) {
    ...
    try {
        ...
        throw new Error('Failed to build app. Make sure you have a build script in package.json');
    }
}
/**
 * Deploys a bundle to an existing Tonk server
 */
async function deployBundle(bundleName, serverName, tonkConfig, packageJson) {
    ...
    try {
        ...
        formData.append('deployData', JSON.stringify({
            bundleName,
            serverName,
            deployType: 'bundle',
            tonkConfig,
...
async function handleDeployCommand(options) {
    ...
    try {
        ...
        const duration = Date.now() - startTime;
        trackCommandError('deploy', error, duration, {
            name: options.name,
            region: options.region,
        });
...
export const deployCommand = new Command('deploy')
    ...
    .option('-n, --name <name>', 'Name for the deployed bundle (defaults to package.json name)')
