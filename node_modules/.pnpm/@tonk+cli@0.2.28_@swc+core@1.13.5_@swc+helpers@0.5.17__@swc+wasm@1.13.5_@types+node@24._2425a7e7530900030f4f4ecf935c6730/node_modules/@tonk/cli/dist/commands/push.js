 ...
 */
async function validateSourceDirectory(sourcePath) {
    if (!fs.existsSync(sourcePath)) {
        console.error(chalk.red(`Error: Directory ${sourcePath} does not exist.`));
        console.log(chalk.yellow(`Run 'npm run build' to create the dist directory.`));
 ...
 */
function determineBundleName(projectRoot, providedName) {
    let defaultName;
    try {
 ...
 */
async function buildWithBasePath(projectRoot, route) {
 ...
 */
async function createTarball(sourcePath, projectRoot, bundleName) {
    console.log(chalk.blue(`Packaging contents of directory ${sourcePath} as bundle '${bundleName}'...`));
    const tempTarPath = path.join(projectRoot, `${bundleName}-${Date.now()}.tar.gz`);
 ...
 */
async function uploadBundle(serverUrl, bundleName, tarballPath) {
    ...
    console.log(chalk.green(`Bundle '${result.bundleName}' uploaded successfully!`));
    return result;
}
/**
 * Starts the bundle on the Tonk server
 */
async function startBundle(serverUrl, bundleName, route) {
    console.log(chalk.blue(`Starting bundle '${bundleName}'...`));
    const requestBody = { bundleName };
 ...
 */
async function handlePushCommand(options) {
    ...
    try {
        ...
        // Create tarball
        const tarballPath = await createTarball(sourcePath, projectRoot, bundleName);
        // Upload bundle
        ...
        if (!options.noStart) {
            ...
            console.log(chalk.blue(`Use 'tonk start ${uploadResult.bundleName} --route ${route}' to start the bundle.`));
        }
        ...
        const duration = Date.now() - startTime;
        trackCommandError('push', error, duration, {
            serverUrl,
            sourceDir,
            name: options.name,
            noBuild: options.noBuild,
...
export const pushCommand = new Command('push')
    ...
    .option('-n, --name <name>', 'Name for the bundle (defaults to directory name)')
