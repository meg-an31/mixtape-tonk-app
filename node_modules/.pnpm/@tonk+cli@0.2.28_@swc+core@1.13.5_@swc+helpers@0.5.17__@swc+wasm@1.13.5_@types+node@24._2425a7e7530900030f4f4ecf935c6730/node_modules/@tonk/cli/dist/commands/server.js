 ...
 */
async function checkDeploymentService() {
    try {
        ...
        console.error(chalk.red('Error: Tonk deployment service is not available.'));
        console.log(chalk.yellow('Please reach out for support or try again later.'));
 ...
 */
async function getServerName(serverName) {
    if (serverName) {
        return serverName;
    }
...
async function createServer(serverName, options) {
    ...
    try {
        ...
        console.log(chalk.yellow(`   Use this server name for future bundle deployments: ${serverName}`));
    }
    catch (error) {
 ...
 */
async function listRemoteBundles(serverName) {
    // Get auth token
    const tonkAuth = await getTonkAuth();
    const authToken = await tonkAuth.getAuthToken();
    if (!authToken) {
 ...
 */
async function listRunningBundles(serverName) {
    // Get auth token
    const tonkAuth = await getTonkAuth();
    const authToken = await tonkAuth.getAuthToken();
    if (!authToken) {
 ...
 */
async function deleteRemoteBundle(bundleName, serverName) {
    // Get auth token
    const tonkAuth = await getTonkAuth();
    const authToken = await tonkAuth.getAuthToken();
    if (!authToken) {
    ...
    const formData = new FormData();
    formData.append('deleteData', JSON.stringify({
        bundleName,
        serverName,
        deployType: 'delete',
    }));
 ...
 */
async function handleServerCreateCommand(options) {
    ...
    try {
        ...
        const duration = Date.now() - startTime;
        trackCommandError('server-create', error, duration, {
            name: options.name,
            region: options.region,
        });
...
const serverCreateCommand = new Command('create')
    ...
    .option('-m, --memory <memory>', 'Memory allocation (e.g., 256mb, 1gb)', config.deployment.defaultMemory)
    .option('-c, --cpus <cpus>', 'Number of CPUs', config.deployment.defaultCpus)
    .option('--remote', 'Use remote Docker build (slower but works with limited local resources)')
...
const serverLsCommand = new Command('ls')
    ...
    try {
        ...
        const duration = Date.now() - startTime;
        trackCommandError('server-ls', error, duration, {
            server: options.server,
        });
        console.error(chalk.red(`Error: ${error instanceof Error ? error.message : String(error)}`));
        await shutdownAnalytics();
...
const serverPsCommand = new Command('ps')
    ...
    try {
        ...
        const duration = Date.now() - startTime;
        trackCommandError('server-ps', error, duration, {
            server: options.server,
        });
        console.error(chalk.red(`Error: ${error instanceof Error ? error.message : String(error)}`));
        await shutdownAnalytics();
...
const serverDestroyCommand = new Command('destroy')
    ...
    try {
        ...
        try {
            ...
            console.error(chalk.red(`Error fetching servers: ${error instanceof Error ? error.message : String(error)}`));
            await shutdownAnalytics();
            process.exitCode = 1;
            return;
        }
        ...
        const duration = Date.now() - startTime;
        trackCommandError('server-destroy', error, duration, {});
        console.error(chalk.red(`Error: ${error instanceof Error ? error.message : String(error)}`));
        await shutdownAnalytics();
        if (tonkAuth)
            tonkAuth.destroy();
...
const serverRmCommand = new Command('rm')
    ...
    try {
        ...
        const duration = Date.now() - startTime;
        trackCommandError('server-rm', error, duration, {
            bundleName,
            server: options.server,
        });
        console.error(chalk.red(`Error: ${error instanceof Error ? error.message : String(error)}`));
...
// Main server command
export const serverCommand = new Command('server').description('Manage Tonk servers');
