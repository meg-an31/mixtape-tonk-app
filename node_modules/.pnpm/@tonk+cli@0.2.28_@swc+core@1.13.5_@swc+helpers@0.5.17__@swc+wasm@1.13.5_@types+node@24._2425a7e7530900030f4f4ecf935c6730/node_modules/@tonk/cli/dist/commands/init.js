...
async function resolvePackagePath(relativePath) {
    try {
        ...
        if (await fs.pathExists(localPath)) {
            ...
            const globalPath = path.join(globalNodeModules, '@tonk/create', relativePath);
            if (await fs.pathExists(globalPath)) {
                return globalPath;
            }
            else {
                throw new Error(`Could not locate ${relativePath} in local or global paths`);
            }
        }
    }
    catch (error) {
...
export const initCommand = new Command('init')
    ...
    .argument('[<directory>]', 'optionally creates a directory and initializes the tonk repo in that directory')
    ...
    try {
        ...
        trackCommandError('init', error, duration, {
            hasDirectory: !!dirPath,
            errorType,
            targetPath: dirPath ? path.join(process.cwd(), dirPath) : process.cwd(),
        });
        console.error(chalk.red('❌ Failed to initialize Tonk repository:'), error);
        await shutdownAnalytics();
        process.exitCode = 1;
    }
});
