import { TonkServer } from '../index.js';
import fs from 'fs';
...
describe('TonkServer', () => {
    ...
    describe('upload-bundle endpoint', () => {
        ...
        beforeEach(async () => {
            // Create test files
            fs.writeFileSync(path.join(testBundleDir, 'index.html'), '<html><body>Test bundle</body></html>');
            fs.writeFileSync(path.join(testBundleDir, 'app.js'), 'console.log("Test bundle");');
        ...
        it('should upload and extract a bundle', async () => {
            ...
            expect(fs.existsSync(path.join(bundlesDir, 'test-bundle', 'test-bundle-files', 'index.html'))).toBe(true);
            expect(fs.existsSync(path.join(bundlesDir, 'test-bundle', 'test-bundle-files', 'app.js'))).toBe(true);
        });
        ...
        it('should handle race conditions with same bundle name', async () => {
            ...
            fs.writeFileSync(path.join(testBundleDir2, 'index.html'), '<html><body>Second bundle</body></html>');
            fs.writeFileSync(path.join(testBundleDir2, 'app.js'), 'console.log("Second bundle");');
            ...
            const hasFiles1 = fs.existsSync(path.join(extractedDir, 'test-bundle-files'));
            const hasFiles2 = fs.existsSync(path.join(extractedDir, 'test-bundle-files-2'));
        ...
        it('should sequentially process uploads with unique identifiers to prevent conflicts', async () => {
            ...
            const testUploads = await Promise.all(Array(5)
                .fill(null)
                .map(async () => {
                ...
                // Create test files
                fs.writeFileSync(path.join(testDir, 'index.html'), `<html><body>Bundle ${uid}</body></html>`);
                fs.writeFileSync(path.join(testDir, 'app.js'), `console.log("Bundle ${uid}");`);
            ...
            const promises = testUploads.map(({ tarPath }) => request
                .post('/upload-bundle')
    ...
    describe('bundle server management', () => {
        ...
        beforeEach(async () => {
            ...
            fs.writeFileSync(path.join(testBundlePath, 'index.html'), '<html><body>Test server</body></html>');
        });
        ...
        it('should list running servers', async () => {
            ...
            const server = psResponse.body.find((s) => s.id === serverId);
