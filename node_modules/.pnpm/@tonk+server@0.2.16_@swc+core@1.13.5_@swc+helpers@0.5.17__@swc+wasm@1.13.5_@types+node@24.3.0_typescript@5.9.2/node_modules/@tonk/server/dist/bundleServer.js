...
export class BundleServer {
    ...
    setupApiProxies() {
        const apiServicesPath = path.join(this.config.bundlePath, 'apiServices.json');
        // Check if apiServices.json exists
        if (!fs.existsSync(apiServicesPath)) {
            this.log('yellow', 'No apiServices.json found. Skipping API proxy setup.');
            return;
        }
        try {
            ...
            if (!Array.isArray(apiServices) || apiServices.length === 0) {
                this.log('yellow', 'apiServices.json is empty or not an array. Skipping API proxy setup.');
                return;
            }
            ...
            for (const service of apiServices) {
                ...
                if (!prefix || !baseUrl) {
                    this.log('yellow', `Skipping invalid API service: ${JSON.stringify(service)}`);
                    continue;
                }
                ...
                if (requiresAuth && authType === 'query' && authQueryParamName) {
                    ...
                    router.use((req, _res, next) => {
                        ...
                        if (this.config.verbose) {
                            this.log('blue', `Added query param ${authQueryParamName} to request: ${req.url}`);
                        }
                        next();
                    });
                    ...
                    this.app.use(proxyPath, router, createProxyMiddleware({
                        target: baseUrl,
                        changeOrigin: true,
                        pathRewrite: {
            ...
            this.log('green', `API proxies set up for ${apiServices.length} services`);
        }
        catch (error) {
    ...
    setupRoutes() {
        ...
        const wsProxy = createProxyMiddleware({
            ...
            onProxyReqWs: (_proxyReq, _req, _socket) => {
    ...
    async updateWasmCache() {
        try {
            ...
            this.log('green', `Service worker updated to cache WASM file: ${wasmFile}`);
        }
        catch (error) {
    ...
    async start() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.server
                ...
                this.log('green', `Bundle server for ${this.config.bundleName} started on port ${this.config.port}`);
                resolve();
            })
                .on('error', err => {
    ...
    stop() {
        return new Promise(resolve => {
            ...
            this.server.close(() => {
                ...
                this.log('yellow', `Bundle server for ${this.config.bundleName} stopped`);
                resolve();
            });
        });
    }
...
//# sourceMappingURL=bundleServer.js.map