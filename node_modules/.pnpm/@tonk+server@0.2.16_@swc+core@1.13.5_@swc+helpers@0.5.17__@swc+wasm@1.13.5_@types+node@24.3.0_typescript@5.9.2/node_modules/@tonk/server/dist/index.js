...
export class TonkServer {
    ...
    repo;
    fsSyncTimer = null;
    bundleServers = new Map();
    bundleRoutes = new Map();
    upload;
    rootNode;
    bundlePersistence;
    constructor(options) {
        ...
        this.rootNode = new RootNode(this.options.persistencePath
    ...
    async initRoot() {
        try {
            ...
            if (rootId) {
                const rootHandle = this.repo.find(rootId);
            ...
            throw new Error('Failed to initialize the root, this error is fatal. Shutting down server.');
        }
    }
    async restorePersistedBundles() {
        try {
            ...
            const { valid: validRoutes, removed: removedBundles } = await this.bundlePersistence.validateAndFilterBundles(persistedRoutes);
            if (removedBundles.length > 0) {
                this.log('yellow', `Removed ${removedBundles.length} missing bundles: ${removedBundles.join(', ')}`);
            }
            ...
            for (const [id, bundle] of validRoutes.entries()) {
                try {
                    ...
                    const existingRoute = Array.from(this.bundleRoutes.values()).find(r => r.route === bundle.route);
                    if (existingRoute) {
                        this.log('yellow', `Route ${bundle.route} is already in use, skipping bundle ${bundle.bundleName}`);
                        continue;
                    }
                    // Setup the bundle route
                    this.setupBundleRoute(bundle.bundleName, bundle.bundlePath, bundle.route, bundle.id);
                    // Restore to the current bundle routes map
                    this.bundleRoutes.set(id, bundle);
                    this.log('green', `Restored bundle ${bundle.bundleName} on route ${bundle.route}`);
                }
                catch (error) {
                    this.log('red', `Failed to restore bundle ${bundle.bundleName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
            this.log('green', `Successfully restored ${this.bundleRoutes.size} bundles`);
        }
        catch (error) {
            this.log('red', `Error restoring persisted bundles: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    setupExpressMiddleware() {
        ...
        this.app.post('/upload-bundle', this.upload.single('bundle'), async (req, res) => {
            try {
                if (!req.file) {
                    return res.status(400).send({ error: 'No bundle file uploaded' });
                }
                const bundleName = req.body.name || path.basename(req.file.originalname, '.tar.gz');
                ...
                try {
                    ...
                    this.log('green', `Bundle ${bundleName} uploaded and extracted to ${bundlePath}`);
                    res.status(200).send({
                        success: true,
            ...
            try {
                ...
                if (bundleRoute) {
                    ...
                    this.bundlePersistence
                        .saveBundleRoutes(this.bundleRoutes)
                        .catch(err => {
                        this.log('red', `Failed to persist bundle routes after removal: ${err.message}`);
                    });
    ...
    setupBundleRoute(bundleName, bundlePath, route, _serverId) {
    ...
    setupBundleApiProxies(bundlePath, route) {
        ...
        try {
            ...
            for (const service of apiServices) {
                const { prefix, baseUrl, requiresAuth, authType, authHeaderName, authEnvVar, authQueryParamName, } = service;
                if (!prefix || !baseUrl) {
                    continue;
                }
                        ...
                        proxyReq: (proxyReq, req, _res) => {
                            ...
                            if (requiresAuth && authType) {
                                ...
                                if (authType === 'apikey' && authHeaderName) {
                                    ...
                                    proxyReq.setHeader('Authorization', `Bearer ${authValue || ''}`);
                                }
                                else if (authType === 'basic') {
                                    proxyReq.setHeader('Authorization', `Basic ${authValue || ''}`);
                                }
                            ...
                            if (this.options.verbose) {
                                this.log('blue', `Proxying request: ${req.method} ${req.url} -> ${baseUrl}`);
                            }
                        },
                        error: (err, _req, res) => {
                            this.log('red', `API proxy error for ${prefix}: ${err.message}`);
