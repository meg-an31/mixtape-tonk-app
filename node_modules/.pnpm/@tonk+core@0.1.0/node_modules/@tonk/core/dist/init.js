 ...
 * ```
 */
export async function initializeTonk(config) {
 ...
 * @param config - Optional configuration for initialization
 */
export async function initializeTonkWithEmbeddedWasm(config) {
    // If already initialized, return immediately
    if (initialized)
        return;
    // If initialization is in progress, return the existing promise
    ...
    initializationPromise = (async () => {
        try {
            if (config?.wasmPath) {
                ...
                const wasmBlob = Uint8Array.from(atob(WASM_BASE64), c => c.charCodeAt(0));
                initSync(wasmBlob);
            }
            initialized = true;
        }
...
export function ensureInitialized() {
    if (!initialized) {
        throw new Error('Tonk WASM module not initialized. Please call initializeTonk() before using any Tonk functionality.\n\n' +
            'Example:\n' +
